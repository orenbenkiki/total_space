...
$PYTHON3 -m total_space.simple_model -f server -c 1 dot -l 'Focus on Server (w/ One Client)' \
| dot -Tpng > images/focus.server.1.png

$PYTHON3 -m total_space.simple_model -f server dot -l 'Focus on Server (w/ Two Clients)' \
| dot -Tpng > images/focus.server.2.png

$PYTHON3 -m total_space.simple_model -f server dot -c server -l 'Cluster Focus on Server' \
...
$PYTHON3 -m total_space.simple_model -n -f server -c 1 dot -m -M -l 'Server Detail (w/ One Client)' \
| dot -Tpng > images/detail.server.1.png

$PYTHON3 -m total_space.simple_model -n -f server dot -m -M -l 'Server Detail (w/ Two Clients)' \
